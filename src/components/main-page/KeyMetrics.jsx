...
import { keyMetric } from "../../content/main-page/key-metrics";
...
export default function KeyMetrics() {
  ...
  const animations = [
    {
      ...
      options: { className:"absolute w-full max-w-[90%]      lg:left-[5%] bottom-[20%] pointer-events-none", animation: { x: -300, opacity: 0 }, delay: 0.1 },
    },

    {
      element: <Image className=" w-full h-auto" src={img_2} alt="circle" />,

      options: { className:"absolute w-full max-h-[201px] left-0 bottom-0 pointer-events-none", animation: { scale: 1.5, opacity: 0 }, delay: 0.2 },
    },
    ...
    {
      element: (
           <Image alt="Load animation" src={img_4} className="w-full h-auto max-w-[80%]" />
       ),
  ...
  const listMap = keyMetric.map((item, index) => ({
    ...item,
      ...
      <div className="grid lg:grid-cols-2  auto-rows-fr gap-[1rem] mt-[3rem]">
        {listMap.map((item, index) => (
          <FadeIn start="top bottom"  key={index}>
            <div className={`p-[1.2rem] relative overflow-hidden flex flex-col   h-full max-h-[320px]  bg-[#1E1E1E]  rounded-[1.2rem] ${index!==1||index!==0?"justify-between":"justify-start"}`}>
              <div>
